...
private:
	...
	static connection_pool<dsn>* get_connection_pool(void)
	{
		...
		if (!ref->m_pool)
			Clog::error_throw(errors::unknown_dsn, "dsn(%d) not reg!", dsn);
...
public:
		...
		unsigned int port = 3306, const char* charset = "gbk",  unsigned int idel = 3 * 60)
	{
		if (!dbname || !server || !user || !password)
			Clog::error_throw(errors::bad_params, "init dsn(%d) params error!",dsn);

		refence<dsn>* ref = mysql::get_ref<dsn>();
		std::lock_guard<std::mutex> lock(ref->m_mutex);
		if(ref->m_pool)
			Clog::error_throw(errors::duplicate_dsn, "init dsn(%d) duplicate!", dsn);
		...
		catch (std::exception& e)
		{
			delete ref->m_pool;
			ref->m_pool = nullptr;
			Clog::error_throw(errors::connect_fail,"dsn(%d) %s" ,dsn,e.what());
		}
	}
